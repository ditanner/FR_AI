...
def play_racer_RL(racer, is_learning, print_state=False):
    racer.draw_hand()
    s = game.current_observation()
    a1 = RL.choose_action(s, is_learning)
...
def play_game(is_learning, rList=[], use_RL=True, print_state=False):
    ...
    while j < 99:
        ...
        if use_RL:
            s, a1 = play_racer_RL(game.players[0].racers[1], is_learning, print_state)
            s2, a2 = play_racer_RL(game.players[0].racers[0], is_learning, print_state)
        else:
...
if __name__ == '__main__':
    ...
    for i in range(num_tests):
        play_game(False, r_list, use_RL=True)
        play_game(True, r_m_list, use_RL=False)
        ...
        if counter % 1000 == 0:
            #            print('Current Table')
            #           print(RL.q_table)
            print(i + 1, 'out of', num_tests, 'tests completed')
    ...
    print('Score over time: ' + str(sum(r_list) / (num_episodes + num_tests)))
    plt.pause(0.1)
